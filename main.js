...
function getTodos() {
  ...
  function errorHandling() {
   axios.get('https://jsonplaceholder.typicode.com/todos').then(res=>showOutput(res)).catch(err=>{
    if(err.response){
  ...
  function cancelToken() {
    ...
    if(true){
       source.cancel('Requested cancelled!')
    }
    }
  
  // INTERCEPTING REQUESTS & RESPONSES
  axios.interceptors.request.use(config=>{
    console.log(`${config.method.toUpperCase()} request sent to ${new data().getTime()}`)
    return config
  },error=>{
